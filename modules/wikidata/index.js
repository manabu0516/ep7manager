...
class AliasBuilder {
    ...
    handle(data) {
        if(data.startsWith("+")) {
            this.target = data.substring(1);
        } else if(this.target != '') {
            this.result[data] = this.target;
        }
    };
...
module.exports = async (parameter) => {    
    ...
    const listFile = await parameter.lib.fs.readFile(parameter.moduleDir + '/list.txt', 'utf8');
    const aliasFile = await parameter.lib.fs.readFile(parameter.moduleDir + '/alias.txt', 'utf8');

    const aliasData = new AliasBuilder();
    aliasFile.split("\r\n").forEach(e => {
        const data = e.trim();
        if(data === '') {
            return;
        ...
        aliasData.handle(data);
    });
    const aliasMap = aliasData.get();

    const entires = {};
    listFile.split('\n').map(line => line.trim().split(',')).forEach(pair => {
        if(pair[0].startsWith("#") === true || pair[0] === '') {
            return;
        }
        entires[pair[0]] = pair[1];
    });

    const listData = Object.keys(entires).map(e => {
        return e + '.json';
    });
    ...
    builder.addApi('entries', () => {
        return Object.keys(entires);
    });

    builder.addApi('normalize', (key) => {
        const data = aliasMap[key];
